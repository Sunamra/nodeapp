...
class ZephyrToast {
	...
	/**
	 * Trim long toast messages
	 */
	trimMessage(msg, len = 20) {
		if (msg.length > len) {
	...
	createToast(message, options = {}) {
		...
		toast.className = `zephyr-toast-notification zephyr_animate__animated ${this.animations[toastOptions.animation.in]
			}`;
		...
		if (toastOptions.icon) {
			...
			// If it's an object with specific properties
				...
				if (toastOptions.icon.url) {
					// Image URL
					iconDiv.innerHTML = `<img src="${toastOptions.icon.url
						}" alt="icon" style="width: ${toastOptions.icon.width || '16px'
		...
		messageDiv.title = toastOptions.message;
